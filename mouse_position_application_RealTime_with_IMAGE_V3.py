...
def callback(event):
    global image
    image = ImageTk.PhotoImage(Image.open(pointer_))
    img_p = canvas.create_image(event.x, event.y, image=image)
...
def mouse_position(event):
   ...
   canvas.create_text(x2+0,y2-25,text=f"{m}", anchor='e', font=("Courier", 15), fill='#02cf17')

# Create a canvas widget
canvas = Canvas(root, width=f"{ws}", height=f"{hs}", bg = '#02cf19', highlightthickness = 0)
canvas.pack()


image = ImageTk.PhotoImage(Image.open(pointer_))
img_p = canvas.create_image(int(f"{ws}")/2, int(f"{hs}")/2, anchor=NW, image=image)
canvas.master.wm_attributes("-transparentcolor","#02cf19")
